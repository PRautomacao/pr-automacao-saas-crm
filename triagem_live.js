...
const GREETING_ONLY = [
  'oi','ola','ol├í','bom dia','boa tarde','boa noite','tudo bem','hey','e ai','e a├¡'
].map(norm);

const ACK_ONLY = [
  'ok','certo','entendi','sim','beleza','perfeito','otimo','├│timo','combinado','blz','show','recebi'
].map(norm);

const FAREWELL_ONLY = [
  'tchau','obrigado','obrigada','ate logo','at├® logo','valeu','muito obrigado','muito obrigada'
].map(norm);
...
function looksLikeStandaloneExamCandidate(value) {
  ...
  const knownShortExamForms = [
    ...
    'calcio','c├ílcio','t4l','fosforo','f├│sforo'
  ].map(norm);
...
function splitPotentialExams(value) {
  ...
  for (const line of lines) {
    let cleaned = line.replace(/^[-ÔÇó*]\s*/g, '').replace(/^\d+[.)-]?\s*/g, '').trim();
...
function extractExamTerms() {
  ...
  // 1) Linhas expl├¡citas: "Exame de Ureia"
  raw
    .replace(/\r/g, '\n')
    .split('\n')
    .map(v => v.trim())
  ...
  // 2) Repeti├º├Áes "um exame de X, um exame de Y, um exame de Z"
...
// ÔöÇÔöÇ confirma├º├úo de exame amb├¡guo ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
const refersPrev = [
  'esses','esses exames','esses ai','esses a├¡','da imagem','do pedido','desses exames'
].includes(text);
...
const s = {
  ...
  asks_do_exam: has([
    ...
    'quero fazer','preciso fazer exame','meu medico me passou','meu m├®dico me passou'
  ]),
...
const isDayKeyword = /\b(segunda|terca|ter├ºa|terca-feira|ter├ºa-feira|quarta|quinta|sexta|sabado|s├íbado|domingo|amanha|amanh├ú|hoje|proxima|pr├│xima|semana que vem)\b/.test(text);
